  ...
  <div>
        ...
        <el-col :span="8" v-for="j in table.cols" :key="j" v-if="isTableData(i, j)">
          <el-card>
            ...
            <div>
              ...
              <p>邮箱：<span>{{getOneDate(i, j).user_email}}</span></p>
            </div>
            <div>
    ...
    <el-dialog title="详情" :visible.sync="dialog.visible" :modal-append-to-body="false" @close="onCancel('form')">
      <el-form :model="form.data" :rules="rules" ref="form">
        <el-row :gutter="10">
          <el-col :span="10">
            <el-form-item label="用户名称" :label-width="form.label_width" prop="user_name">
              <el-input v-model="form.data.user_name" :disabled="form.input_disabled" required=true></el-input>
            </el-form-item>
            <el-form-item label="用户密码" :label-width="form.label_width" prop="password" v-if="!form.input_pw_disabled">
              <el-input v-model="form.data.password" :disabled="form.input_disabled" required=true></el-input>
            </el-form-item>
            <el-form-item label="用户邮箱" :label-width="form.label_width" prop="password" v-if="!form.input_pw_disabled">
              <el-input v-model="form.data.user_email" :disabled="form.input_disabled" required=true></el-input>
            </el-form-item>
          </el-col>
        </el-row>
      </el-form>
...
<script>
import SystemUserOrdinaryApi from '@/apis/skill_learn_assistant/admin/user/OrdinaryApi';
export default {
  name: 'OrdinaryUser',
  data () {
    return {
  ...
  methods: {
    getOrdinaryUserList () {
      ...
      SystemUserOrdinaryApi.getList(params).then(function (result) {
      ...
      this.$refs[form].validate((valid) => {
        if (valid) {
          let params = {
            ...
            user_email: that.form.data.user_email
          };
          if (this.dialog.type === 1) {
            SystemUserOrdinaryApi.add(params).then(function (result) {
              alert(result.data.msg);
              if (result.success) {
                that.dialog.visible = false;
                that.$refs[form].resetFields();
                that.resetFormData();
                that.getOrdinaryUserList();
              }
            }).catch(function (error) {
              console.log(error);
            });
          ...
          if (this.dialog.type === 2) {
            SystemUserOrdinaryApi.del(params).then(function (result) {
              alert(result.data.msg);
              if (result.success) {
                that.dialog.visible = false;
                that.$refs[form].resetFields();
          ...
          if (this.dialog.type === 3) {
            SystemUserOrdinaryApi.edit(params).then(function (result) {
              alert(result.data.msg);
              if (result.success) {
                that.dialog.visible = false;
                that.$refs[form].resetFields();
    ...
    getFormData (row) {
      ...
      this.form.data.user_email = row.user_email;
    },
    resetFormData () {
      this.form.data.user_name = '';
      this.form.data.password = '';
      this.form.data.user_email = '';
    },
    getOneDate (i, j) {
