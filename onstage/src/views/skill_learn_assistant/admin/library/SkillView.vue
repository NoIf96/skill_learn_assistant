...
<script>
import SystemLibrarySkillApi from '@/apis/skill_learn_assistant/admin/library/SkillApi';
export default {
  name: 'SkillLibrary',
  data () {
    return {
      ...
      rules: {
        ...
        sort_language: [
          {required: true, message: '请选择一个语言类别', trigger: 'blur'}
        ]
      }
    }
  },
  ...
  methods: {
    getSortOptions () {
      let that = this;
      SystemLibrarySkillApi.getSortOptions().then(function (result) {
    ...
    handleAutoAllIntroduction () {
      SystemLibrarySkillApi.autoUpdateSkillIntroduction().then(function (result) {
    ...
    onGetIntroduction () {
      ...
      SystemLibrarySkillApi.getSkillIntroduction(params).then(function (result) {
        alert(result.data.msg);
        if (result.success) {
      ...
      this.$refs[form].validate((valid) => {
        if (valid) {
          ...
          if (this.dialog.type === 1) {
            SystemLibrarySkillApi.add(params).then(function (result) {
              alert(result.data.msg);
              if (result.success) {
          ...
          if (this.dialog.type === 2) {
            SystemLibrarySkillApi.del(params).then(function (result) {
              alert(result.data.msg);
              if (result.success) {
                that.dialog.visible = false;
                that.$refs[form].resetFields();
          ...
          if (this.dialog.type === 3) {
            SystemLibrarySkillApi.edit(params).then(function (result) {
              alert(result.data.msg);
              if (result.success) {
                that.dialog.visible = false;
                that.$refs[form].resetFields();
